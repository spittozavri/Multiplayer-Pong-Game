...
io.on('connection', (socket: Socket) => {
  ...
  socket.on(ClientEvents.PADDLE_MOVE, (direction: 'up' | 'down') => {
    const room = roomManager.getPlayerRoom(socket.id);
    if (room && room.game) {
      room.game.movePaddle(socket.id, direction); // Pass socket.id to movePaddle
    }
  });
          ...
          if (initialGameState.isPlaying && !gameLoops.has(room.id)) {
               const interval = setInterval(() => {
                  if (room.game) {
                      ...
                      io.to(room.id).emit(ServerEvents.GAME_STATE_UPDATE, { ...gameState, players: playersArray });
